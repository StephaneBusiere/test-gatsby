import React, { useState, useRef, createRef, useCallback, useEffect, useLayoutEffect } from 'react';
 ...
 */
		...
		var _cssRatioSetter = function(pt, cssp, mod) { //Takes an individual CSSPropTween and converts it into a type:2 that has a setRatio that does everything the regular CSSPlugin.setRatio() method does but applying the mod() too. We do this to keep the main CSSPlugin.setRatio() as fast as possible (the vast majority of times, no mod() will be necessary)
			...
			p$2 = ModifiersPlugin.prototype;

		p$2._add = function(target, p, s, c, mod) {
			this._addTween(target, p, s, s + c, p, mod);
			this._overwriteProps.push(p);
		};

	p$2 = _gsScope._gsDefine.globals.TweenLite.version.split(".");
	if (Number(p$2[0]) <= 1 && Number(p$2[1]) < 19 && _gsScope.console) {
...
var Marquee = function Marquee(props) {
  ...
  var _useState5 = useState(false),
      ...
      separation = _useState6[0],
      setSeparation = _useState6[1];

  var target = useRef(cards.map(function () {
    return createRef();
  }));
  var speed = 20;
  var isScrolling = null;
  var reverse = false;
  var tweenDistance = props.reverse ? separation * cards.length : -separation * cards.length;
  var sensitivity = 0.001;
  var animate = useCallback(function (e, i) {
    for (var _i = 0; _i < cards.length; _i++) {
      TweenMax$1.set(target.current[_i], {
        x: (_i + cards.length) * separation
      });
      TweenMax$1.to(target.current[_i], speed, {
        repeat: -1,
        ease: Linear.easeNone,
        x: "+=".concat(tweenDistance),
        modifiers: {
          x: function x(_x) {
            return _x % tweenDistance;
          }
        }
      });
    }
  ...
  useLayoutEffect(function () {
    addCards();
    var newSeparation = Math.round(target.current[0].clientWidth);
    setSeparation(newSeparation);
    setDistance(newSeparation * cards.length);
  }, []);

  var addCards = function addCards() {
    var amountCalc = Math.ceil((window.innerWidth + target.current[0].clientWidth) / (target.current[0].clientWidth * cards.length));

    for (var i = cards.length; i < amountCalc; i++) {
      cards.push('');
    }
  };

  var playTween = function playTween() {
    var allTweens = TweenMax$1.getAllTweens();
  ...
  var reverseTween = function reverseTween() {
    var allTweens = TweenMax$1.getAllTweens();
  ...
  window.onscroll = function () {
    var allTweens = TweenMax$1.getAllTweens();
    var newPos = checkScrollSpeed();
    var min = 20 / 13.333; // checks if user is scrolling up or down and sets value accordingly
    ...
    if (allTweens.length) {
      var _allTweens = TweenMax$1.getAllTweens();

      var progress = _allTweens[0].progress();

      if (target) {
        for (var i = 0; i < _allTweens.length; i++) {
          _allTweens[i].pause(); // boost progress to prevent tween from stopping when reversing past 0 progress


          _allTweens[i].progress(9999);

          _allTweens[i].progress(progress + newPos * sensitivity);
        }
      }
    }
  };
  ...
  return React.createElement(React.Fragment, null, React.createElement("div", {
    ...
    style: {
      transform: "translateX(-".concat(target.current[0].clientWidth, "px)"),
      width: "".concat(window.innerWidth + target.current[0].clientWidth, "px"),
      height: "".concat(target.current[0].clientHeight, "px")
    }
    ...
    return React.createElement("div", {
      ...
      className: "marquee",
      ref: function ref(e) {
        return target.current[i] = e;
      },
      style: {
        paddingLeft: "".concat(props.spacing, "px")
      }
    }, props.children);
  }))));
};
